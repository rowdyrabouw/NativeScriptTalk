...
export class MLKitComponent {
  ...
  private mlkitOnDeviceFeatures: Array<string> = ["Text recognition", "Barcode scanning", "Face detection", "Image labeling"];

  constructor(private routerExtensions: RouterExtensions, private zone: NgZone) {}

  fromCameraFeed(): void {
    action("Test which on-device ML Kit feature?", "Cancel", this.mlkitOnDeviceFeatures).then((pickedItem: string) => {
      let to;
      if (pickedItem === "Text recognition") {
        to = "/mlkit/textrecognition";
      } else if (pickedItem === "Barcode scanning") {
        to = "/mlkit/barcodescanning";
      } else if (pickedItem === "Face detection") {
        to = "/mlkit/facedetection";
      } else if (pickedItem === "Image labeling") {
        to = "/mlkit/imagelabeling";
      }
  ...
  private scanBarcodeOnDevice(imageSource: ImageSource): void {
    ...
    firebase.mlkit.barcodescanning
      .scanBarcodesOnDevice({
        image: imageSource,
  ...
  private detectFacesOnDevice(imageSource: ImageSource): void {
    firebase.mlkit.facedetection
      ...
      .then((result: MLKitDetectFacesOnDeviceResult) => {
        alert({
          title: `Result`,
