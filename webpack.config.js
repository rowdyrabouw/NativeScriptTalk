...
module.exports = env => {
  ...
  const config = {
    ...
    entry: {
      bundle: aot ? `./${nsWebpack.getAotEntryModule(appFullPath)}` : `./${nsWebpack.getEntryModule(appFullPath)}`,
      vendor: "./vendor"
    },
    output: {
      pathinfo: true,
    ...
    module: {
      rules: [
        ...
        {
          test: /.ts$/,
          use: ["nativescript-dev-webpack/moduleid-compat-loader", { loader: "@ngtools/webpack", options: ngToolsWebpackOptions }]
        }
      ]
    },
    plugins: [
